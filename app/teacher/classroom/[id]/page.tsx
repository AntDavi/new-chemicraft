  ...
  const studentStats = enrollments.map((enrollment) => {
    ...
    const lastActivityDate =
      studentSessions.length > 0
        ? new Date(
            Math.max(...studentSessions.map((s) => new Date(s.started_at).getTime())),
          )
        : null

    const isInactive =
      !lastActivityDate || lastActivityDate < sevenDaysAgo
    ...
    <div className="min-h-screen bg-muted">
      ...
      <main className="max-w-2xl mx-auto px-4 py-8 space-y-6">
        ...
        <div className="grid grid-cols-3 gap-3">
          ...
          <Card
            className={
              inactiveCount > 0 ? 'border-amber-400/60 bg-amber-50/60 dark:bg-amber-950/20' : ''
            }
          >
            <CardContent className="pt-4 pb-3">
        ...
        <div className="space-y-3">
          ...
          {enrollments.length === 0 ? (
            ...
            <div className="space-y-2">
              {studentStats.map(
                ({ enrollment, total, completed, lastActivityDate, isInactive }) => {
